...
At the end of every semester your professor for "Introduction to Databases" saves the exam results of every student
 in a simple database system. In the database table Grades, there are five columns:
...
As a Teaching Assistant (TA), you need to query the name and id of all the students whose best grade comes
from Option 3, sorted based on the first 3 characters of their name. If the first 3 characters of two names
are the same, then the student with the lower ID value comes first.

*/

CREATE PROCEDURE gradeDistribution()
BEGIN
	SELECT tabela.name as Name, tabela.ID as ID
    FROM (
        SELECT name, ID, 
        ...
        FROM Grades
         ...
         WHERE tabela.option_3 > tabela.option_2 and tabela.option_3 > tabela.option_1
