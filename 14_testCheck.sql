...
Your task is to return the table with a column id and a column checks, where for each answers id the following
...
CREATE PROCEDURE testCheck()
    SELECT id, IF (correct_answer=given_answer, "correct",
               IF (ISNULL(correct_answer) or ISNULL(given_answer),"no answer","incorrect") ) AS checks
    FROM answers
    ORDER BY id;